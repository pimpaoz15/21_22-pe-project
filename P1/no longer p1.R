...
ggplot(data_transformed, aes(x = tempo)) + 
  ...
  labs(x = "Year", y = "Standardized Value",
       title = "Evolution of GCP and TPP from 2000") +
  theme_bw()

print(g)

# Save graph as a PDF file
ggsave("Exercise 1", width = 8, height = 4)

# To run, open a new terminal in VSCode Command + Enter to process Lines of code
# All data will be saved on the Global Environment, under Workspace in the R code section
